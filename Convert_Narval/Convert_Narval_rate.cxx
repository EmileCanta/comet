  ...
  if(fich_paramlect2.good())
    {
		while(!fich_paramlect2.eof())
	      ...
	      if(buffer != "")
					...
					//déclaration et initialisation de idx[nb_slot][n0channel]
					int idx[3][6];
					...
					idx[3][4]=17;
					idx[3][5]=18;
					det_num.nomdetect=idx[slot][channel];
  ...
  while (narval.good())
    {
      ...
      while ((word(swap,word1[0],word1[1])==0xffff) && (word(swap,word2[0],word2[1])==0x8))
				...
				if(ser == 0) // checking it is no a service hit
					...
					//TBranch *idx2 = input->GetBranch(nomdetector);
					
					//if(idx != 1)
					//{
					     cout << idx->GetEntries() << endl;
					...
					if(idx)
						{
							...
							if(tm > 1.e-8 && (buffer_idx[nbrevent2] < nbrdetect + 1) && (buffer_idx[nbrevent2] != 0)) nbrcoup_detect[det_num.nomdetect]++;
							...
							if((nbrevent2 ==  (memory_nbrevent_coding + 2*nbrdetect)) && codenvu == 1)
								...
								// Normalement de 0 à position1erevenement_postcoden-1 j'ai tout les evenements pre coden
